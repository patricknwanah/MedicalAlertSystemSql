...
drop procedure DATA.addBp
go
create procedure DATA.addBp @PatientName varchar(50), @BloodPressureHigh int, @BloodPressureLow int
as
	declare @UserID int
...
drop procedure DATA.addBg
go
create procedure DATA.addBg @PatientName varchar(50), @BloodGlucoseLevel varchar(50)
as
	declare @UserID int
	set @UserID = (select userid from DATA.Users where loginname = @PatientName)
...
drop procedure DATA.addHr
go
create procedure DATA.addHr @PatientName varchar(50), @HeartRate varchar(50)
as
	declare @UserID int
	set @UserID = (select userid from DATA.Users where loginname = @PatientName)
	insert into data.HeartRate(UserID,HeartRate) values (@UserID,@HeartRate)
go
...
drop procedure DATA.setCareGiverPermissions
go
create procedure DATA.setCareGiverPermissions @PatientName varchar(50), @CareGiverName varchar(50), @bp int, @bg int, @hr int
as 
	Declare @UserID int
	set @UserID = (select userid from data.Users where loginname = @PatientName)
	Declare @CareGiverID int
	...
	Update DATA.PatientCaretakers set canViewBp = @bp , canViewBGp = @bg, canViewHr = @hr where UserID = @UserID and CaretakerID = @CareGiverID
go



drop procedure DATA.getCareGiverPermissions
go
create procedure DATA.getCareGiverPermissions @PatientName varchar(50), @CareGiverName varchar(50)
as 
	Declare @UserID int
	set @UserID = (select userid from data.Users where loginname = @PatientName)
	Declare @CareGiverID int
	...
	select canViewBp , canViewBGp , canViewHr from DATA.PatientCaretakers where UserID = @UserID and CaretakerID = @CareGiverID
go

exec data.setCareGiverPermissions'Godmode', 'Salona1', 1,1,1
exec data.getCareGiverPermissions 'Godmode', 'Salona1'
